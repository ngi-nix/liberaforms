...
class TestAdminUsers():
    def test_toggle_admin(self, users, client, anon_client, admin_client):
        """ Test permissions
            Test toggle
        """
        url = f"/admin/users/toggle-admin/{users['test_user'].id}"
        ...
        html = response.data.decode()
        assert '<!-- site_index_page -->' in html
        initial_is_admin = users['test_user'].admin['isAdmin']
        ...
        assert response.json == {"admin": users['test_user'].admin['isAdmin']}
        assert initial_is_admin != users['test_user'].admin['isAdmin']
    ...
    def test_toggle_uploads(self, users, client, anon_client, admin_client):
        """ Test permissions
            Test toggle
        """
        url = f"/admin/users/toggle-uploads-enabled/{users['test_user'].id}"
        response = anon_client.post(
                        url,
                        follow_redirects=True,
                    )
        ...
        html = response.data.decode()
        assert '<!-- site_index_page -->' in html
        initial_uploads_enabled = users['test_user'].uploads_enabled
        response = admin_client.post(
                        url,
                        follow_redirects=False,
                    )
        assert response.status_code == 200
        assert response.is_json == True
        assert response.json == {"uploads_enabled": users['test_user'].uploads_enabled}
        assert initial_uploads_enabled != users['test_user'].uploads_enabled
        # set user.uploads_enabled to False to continue testing
        users['test_user'].uploads_enabled = False
        users['test_user'].save()
