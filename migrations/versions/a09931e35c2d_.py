...
class Form(Base):
    ...
    shared_notifications = sa.Column(MutableList.as_mutable(ARRAY(sa.String)), nullable=True)
...
class FormUser(Base):
    ...
    ascending = sa.Column(sa.Boolean, default=True)
...
def upgrade():
    ...
    op.drop_column('forms', 'shared_notifications')
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('forms', sa.Column('editors', postgresql.JSONB(astext_type=sa.Text()),
                                     autoincrement=False, nullable=True))
    op.add_column('forms', sa.Column('sharedAnswers', postgresql.JSONB(astext_type=sa.Text()),
                                     autoincrement=False, nullable=True))
    op.add_column('forms', sa.Column('shared_notifications', postgresql.ARRAY(sa.VARCHAR()),
                                     autoincrement=False, nullable=True, default=[]))

    session = Session(bind=op.get_bind())
    formusers = session.query(FormUser).all()
    for formuser in formusers:
        form = session.query(Form).filter_by(id=formuser.form_id, author_id=formuser.user_id).first()
        if form:
            form.shared_notifications = []
            form.editors = form.editors if form.editors else {}
            form.editors[str(formuser.user_id)] = {'notification':formuser.notifications}
            continue
        form = session.query(Form).filter_by(id=formuser.form_id).first()
        if formuser.is_editor:
            form.editors = form.editors if form.editors else {}
            form.editors[str(formuser.user_id)] = {'notification':formuser.notifications}
    session.commit()
    session.close()

    op.alter_column('forms', 'editors', nullable=False)
    op.alter_column('forms', 'shared_notifications', nullable=False)
