...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    environment.systemPackages = [ liberaforms ];
    ...
    systemd.services.liberaforms = {
      ...
      serviceConfig = {
        Type = "simple";
        PrivateTmp = true;
        ExecStartPre = assert secretKeyFile != null; pkgs.writeScript "liberaforms-init" ''
          #!/bin/sh
          mkdir -p "${workDir}"
          mkdir -p "${default_logs}"
          chown ${user}:${group} "${workDir}" "${default_logs}"
          cat > ${workDir}/.env <<EOF
          # Do not edit this file, it is automatically generated by liberaforms.service

          SECRET_KEY="$(cat "${secretKeyFile}")"
          
          ROOT_USERS = ['${rootEmail}']
        ...
        ExecStart = ''${gunicorn}/bin/gunicorn \
              -c ${workDir}/gunicorn.py 'wsgi:create_app()'
            '';
